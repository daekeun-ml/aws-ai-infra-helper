...
class Float8GroupedMMConverter(QuantizationConverter):
    ...
    class Config(QuantizationConverter.Config):
        _quantization_type: ClassVar[str] = "float8"
        # Note: __name__ is set to "Float8GroupedMMConverterConfig" below the class body
        # to avoid a tyro subcommand naming collision with Float8LinearConverter.Config
        # (tyro uses __name__, not __qualname__, for subcommand disambiguation).
...
# Both Float8LinearConverter.Config and Float8GroupedMMConverter.Config have __name__ = "Config"
# in the same module, causing a naming collision. Rename to make them distinct.
Float8GroupedMMConverter.Config.__name__ = "Float8GroupedMMConverterConfig"
