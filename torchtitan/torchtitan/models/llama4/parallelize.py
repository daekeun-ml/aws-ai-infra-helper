    ...
    for layer_id, transformer_block in model.layers.items():
        ...
        if transformer_block.moe_enabled:
            ...
            if ep_degree == 1 and expert_shard_placement == Shard(0):
                ...
                # Requires PyTorch with ShardPlacementResult support (nightly / >= 2.12)
                assert edp_mesh is not None
                ...
                try:
                    ...
                    _has_shard_placement_result = True
                except ImportError:
                    _has_shard_placement_result = False

                if not _has_shard_placement_result:
                    if efsdp_size > 1:
                        raise RuntimeError(
                            f"Per-param mesh FSDP2 is required when ep_degree > 1 and "
                            f"efsdp > 1 (efsdp={efsdp_size}), but ShardPlacementResult "
                            f"is not available in your PyTorch version. "
                            f"Please upgrade to PyTorch >= 2.12 (nightly)."
                        )
                    # efsdp=1: expert params need no FSDP sharding since EP already
                    # distributes them. Exclude them from FSDP entirely.
                    fully_shard(
                        transformer_block,
                        **fsdp_config,
                        reshard_after_forward=reshard_after_forward,
