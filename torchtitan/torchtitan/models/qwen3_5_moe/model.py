...
_Embedding = Module.from_nn_module(nn.Embedding)
...
class GatedDeltaNet(Module):
    ...
    class Config(Module.Config):
        ...
        norm_eps: float = 1e-6
        fla_backend: str = "fla_chunked" if _HAS_FLA else "torch_naive"
    ...
    def __init__(self, config: Config, *, dim: int, **kwargs):
        ...
        self.in_proj_qkv = _Linear(dim, conv_dim, bias=False)
        self.in_proj_z = _Linear(dim, value_dim, bias=False)
        self.in_proj_a = _Linear(dim, config.n_value_heads, bias=False)
        self.in_proj_b = _Linear(dim, config.n_value_heads, bias=False)
        ...
        self.out_proj = _Linear(value_dim, dim, bias=False)
...
class Attention(BaseAttention):
    ...
    def __init__(self, config: Config, *, dim: int, **kwargs):
        ...
        self.wq = _Linear(dim, self.n_heads * self.head_dim * 2, bias=config.bias)
        self.wk = _Linear(dim, self.n_kv_heads * self.head_dim, bias=config.bias)
        self.wv = _Linear(dim, self.n_kv_heads * self.head_dim, bias=config.bias)
        self.wo = _Linear(self.n_heads * self.head_dim, dim, bias=config.bias)
...
class TransformerBlock(Module):
    ...
    def __init__(
        ...
        self.shared_gate = _Linear(dim, 1, bias=False)
...
class Model(BaseModel):
        ...
        def update_from_config(
            ...
            **kwargs,
            ...
            if self.layer.moe is not None:
                self.layer.moe.router._debug_force_load_balance = debug.moe_force_load_balance
    ...
    def __init__(self, config: Config):
        ...
        self.tok_embeddings = _Embedding(config.vocab_size, config.dim)
