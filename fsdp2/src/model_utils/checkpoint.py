...
def save_checkpoint(model, optimizer, scheduler, user_content, root_dir, sub_dir):
    ...
    if os.path.exists(save_dir):
        if dist.get_rank() == 0:
            logger.info(f"Removing existing checkpoint directory: {save_dir}")
            shutil.rmtree(save_dir)
        dist.barrier()  # Wait for rank 0 to finish removal
    ...
    with FSDP.state_dict_type(
        ...
        dist_cp.save(
                    ...
                    storage_writer=dist_cp.FileSystemWriter(save_dir, overwrite=False)
                )
    dist.barrier()
    if dist.get_rank() == 0:
